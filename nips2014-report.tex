...
Where we used $ \frac{N_{dk}}{N_d} \equiv \bar{z_d} $ and $\mathbb{I}(condition) = 1$ iff the condition evaluates to true, and zero otherwise. 
...
In fact, most words were given a score of exactly \(0.0013665\). 
...
A larger data set could increase performance, but before that can realistically implemented one first needs to speed up the algorithm.

If a sufficient speed up is achieved, one could also look at full movie scripts as input text, rather than plot summaries. 

In conclusion, we can say that using scores is an improvement to the algorithm, but the summaries of movies still form a text corpus that is too big to effectively handle with current algorithms.
...
Jorge S\'{a}ez G\'{o}mez did most of the coding and experimenting, with help from Roelof van der Heijden.
The latter of whom wrote the most of this report, with help from Jorge where needed.
The presentation was written by both of them.

Francesco Stablum, who was initially also a member of this group, did not contribute anything to this project.
